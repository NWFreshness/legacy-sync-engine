...
CREATE TABLE IF NOT EXISTS sync_state (
    ...
    direction TEXT CHECK (direction IN ('modern_to_legacy', 'legacy_to_modern')),
...
CREATE TABLE IF NOT EXISTS audit_log (
    ...
    status TEXT DEFAULT 'applied',  -- applied, conflicted, dead_lettered
    error_message TEXT
);
...
-- Indexes for performance on pollers
...
BEGIN
    ...
    INSERT INTO modern.customers (id, email, name, company, status, updated_at)
    VALUES ('00000000-0000-0000-0000-000000000001'::uuid, 'alice@acme.com', 'Alice Smith', 'Acme Corp', 'active', NOW() - INTERVAL '1 hour')
    ON CONFLICT (id) DO NOTHING;

    -- Legacy version with different data (to trigger conflict)
    INSERT INTO legacy.CUST_MSTR (CUST_ID, CUST_NAME, CUST_EMAIL, CUST_CO, STATUS_CD, LAST_UPD_DT)
    VALUES ('C001', 'Alice Smith Updated', 'alice.new@acme.com', 'ACME INC', 'A', CURRENT_DATE - 1)
    ON CONFLICT (CUST_ID) DO NOTHING;

    -- Similar for orders and inventory...
    INSERT INTO modern.orders (id, customer_id, order_date, total_amount, status)
    VALUES ('00000000-0000-0000-0000-000000000002'::uuid, '00000000-0000-0000-0000-000000000001'::uuid, NOW(), 1250.00, 'shipped');

    INSERT INTO legacy.ORD_HDR (ORD_ID, CUST_ID, ORD_DT, ORD_AMT, ORD_STAT)
    VALUES ('ORD-001', 'C001', CURRENT_DATE, 999.99, 'S');
END;
$$ LANGUAGE plpgsql;
