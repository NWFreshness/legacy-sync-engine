...
-- Overlapping dirty seed (idempotent). Modern statements to modern_saas,
-- legacy statements to legacy_erp.
...
-- LEGACY_BEGIN
-- Alice has a matching legacy row under the UUID-truncated PK (same as the
-- mapping's modern_to_legacy transform produces) so LWW and conflict tests
...
VALUES
  ('CUST_MSTR', '00000000-0000-0000-0', 'U', '{"CUST_ID":"00000000-0000-0000-0","note":"seeded drift"}'::jsonb),
  ('CUST_MSTR', 'C002', 'I', '{"CUST_ID":"C002"}'::jsonb),
  ('CUST_MSTR', 'C666', 'I', '{"CUST_ID":"C666"}'::jsonb),
  ('INV_BAL', 'WIDGET-1', 'U', '{"PROD_CD":"WIDGET-1","QTY_OH":175}'::jsonb);
-- LEGACY_END
